...
- There are various operators used in **MongoDB** to filter documents:-
  ...
  1. **$gte** - Used to query the documents where value of the field is **>=** of a specified value.
     - **Syntax** - `{field: { $gte: value}}`
  1. **$lt** - Used to query the documents where value of the field is **<** of a specified value.
     - **Syntax** - `{field : {$lt : value}}`
  1. **$gt** - Used to query the documents where value of the field is **>** of a specified value.
     - **Syntax** - `{field : {$gt : value}}`
  1. **$or** - Used to query documents based on any of the conditions passed in an array is satisfied.
     - **Syntax** - `{$or : [<condition1>, <condition2>, ...]}`
  1. **$set** - Used to set(replace) value of a field in a document or create a new field.
     - **Syntax** - `{$set: { <field1>: <value1>, ... }}`
  1. **$ne** - Used to query documents which are not equal to given value.
     - **Syntax** - `{field : {$ne : value}}`
  1. **$in** - Used to query documents where mentioned field has the any of the values specified in an array using the `in` operator.
     - **Syntax** - `{field : {$in : [<value1>, <value2>]}}`
  1. **$nin** - negation of `in` operator.
